...
public class DateCenter implements FlipPagerDateInterface {
    ...
    public DateCenter(Context context) {
       strings = new ArrayList<>();
       this.context = context;
       objectObjectLruCache = new LruCache<String, Bitmap>((int)Runtime.getRuntime().maxMemory()/ 1024);
    ...
    public Bitmap currentPage() {
        ...
        bitmap = BitmapUtils.adjustFromFile2Bitmap(strings.get(currentPage), context);
    ...
    public Bitmap nextPage() {
        if (currentPage + 1 < strings.size()){
            ...
            if (bitmap != null)
                return bitmap;
            bitmap = BitmapUtils.adjustFromFile2Bitmap(strings.get(currentPage + 1), context);
